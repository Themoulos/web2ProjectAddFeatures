...
function __extract_from_showtask2($arr, $level, $today_view, $s, $m, $jsTaskId, $expanded)
{
    ...
    if ($arr['task_milestone'] > 0) {
        ...
        $open_link = '<a href="javascript: void(0);"><img onclick="expand_collapse(\'' . $jsTaskId . 
                '\', \'tblProjects\',\'\',' . ($level++) . ');" id="' . $jsTaskId . '_collapse" src="' . w2PfindImage('icons/collapse.gif') . 
                '" class="center" ' . (!$expanded ? 'style="display:none"' : '') . ' /><img onclick="expand_collapse(\'' . $jsTaskId . 
                '\', \'tblProjects\',\'\',' . ($level++) . ');" id="' . $jsTaskId . '_expand" src="' . w2PfindImage('icons/expand.gif') .
                '" class="center" ' . ($expanded ? 'style="display:none"' : '') . ' /></a>';
        $s .= $open_link;
...
{
    return __extract_from_showtask2($arr, $level, $today_view, $listTable, $fields);
}
...
function getFolders($parent)
{
    ...
    foreach ($folders as $row) {
        if (array_key_exists($row['file_folder_id'], $allowed_folders_ary) or array_key_exists($parent, $allowed_folders_ary)) {
            ...
            $s .= '<li><a href="./index.php?m=files&amp;a=addedit_folder&amp;folder=' . $row['file_folder_id'] . '">' . w2PshowImage('filesaveas.png', '', '', 'edit folder', 'edit this folder', 'files') . '</a></li>';
            if ($m == 'files') {
...
function displayFiles($AppUI, $folder_id, $task_id, $project_id, $company_id)
{
    ...
    $allow1 = $project->getAllowedProjects($AppUI->user_id, false);
    if (count($allow1) > 0) {
        $q->addWhere('((file_project IN (' . implode(',', array_keys($allow1)) . ') OR file_project = 0))');
    } else {
        $q->addWhere( '(file_project = 0)');
    }
    //TODO: apply permissions properly, this is not needed, permissions are per PROJECT AND COMPANY
    //$task = new CTask();
    //$allow2 = $task->getAllowedSQL($AppUI->user_id);
    //if (count($allow2) > 0) {
    //    $q->addWhere('file_task IN (' . implode(',', $allow2) . ')');
    //}
...
function w2PtoolTip($header = '', $tip = '', $raw = false, $id = '')
{
    ...
    $id = ('' == $id) ? '' : 'id="' . $id . '"';
    if ($raw) {
        $starttip = '<span ' . $id . ' title="&lt;h4&gt;' . nl2br2($AppUI->_($header)) . '&lt;/h4&gt; ' . nl2br2($AppUI->_($tip)) . '">';
    } else {
        $starttip = '<span ' . $id . ' title="&lt;h4&gt;' . nl2br2(ucwords(strtolower($AppUI->_($header)))) . '&lt;/h4&gt; ' . nl2br2(strtolower($AppUI->_($tip))) . '">';
    }
...
//
// New password code based on code from Mambo Open Source Core
        ...
        for ($i = 0, $i_cmp = $start->dateDiff($end); $i <= $i_cmp; $i++) {
            ...
            if (($row['event_cwd'] && in_array($date->getDayOfWeek(), $cwd)) || !$row['event_cwd']) {
                if ($minical) {
                    ...
                    $link['href'] = '';
                    $link['alt'] = '';
                    $link['text'] = '<span style="color:' . bestColor($rowColor) . ';background-color:#' . $rowColor . '">';
                    $link['text'] .= w2PtoolTip($row['event_name'], getEventTooltip($row['event_id']), true) . w2PshowImage('modules/events/event' . $row['event_type'] . '.png', 16, 16, '', '', 'calendar') . '</a>' . '<a href="' . $url . '">' . $row['event_name'] . '</a></span>' . w2PendTip();
                    $link['color'] = 'white';
                }
...
function getTaskLinks($startPeriod, $endPeriod, $links, $strMaxLen, $company_id = 0, $minical = false, $userid=0)
{
    ...
    foreach ($tasks as $row) {
        ...
        if ($row['task_percent_complete'] = 100) { //HAS FINISHED
            $rowColor = "00FF00";
            $link['color'] = 'green';
        } elseif ($today->after($end)) {
            $rowColor = "FF0000"; //LATE!!!
            $link['color'] = 'red';
        } elseif ($today->after($start) && $today->before($end)) { //OK!!!
            $rowColor = "0000FF"; //SHOULD HAVE STARTED
            $link['color'] = 'blue';
...
function getTaskEndingLinks($startPeriod, $endPeriod, $links, $strMaxLen, $company_id = 0, $minical = false, $userid = 0) {
    global $a, $AppUI;
    $tasks = CTask::getTasksEndingForPeriod($startPeriod, $endPeriod, $company_id, $userid);
    $tf = $AppUI->getPref('TIMEFORMAT');
    //subtract one second so we don't have to compare the start dates for exact matches with the startPeriod which is 00:00 of a given day.
    //$startPeriod->subtractSeconds(1);
    ...
    $today2 = new w2p_Utilities_Date();
    $today2->setTime(23, 59, 59); //END of the day
    //
    // establish the focus 'date'
    $this_week = new w2p_Utilities_Date($today2);
    $dd = $this_week->getDay();
    $mm = $this_week->getMonth();
    $yy = $this_week->getYear();

    // prepare time period for 'events'
    $tommorow1 = new w2p_Utilities_Date(Date_Calc::nextDay($dd, $mm, $yy, FMT_TIMESTAMP_DATE, LOCALE_FIRST_DAY));
    $tommorow1->setTime(23, 59, 59); 

    $today->convertTZ($AppUI->getPref('TIMEZONE'));
    $today2->convertTZ($AppUI->getPref('TIMEZONE'));
    $tommorow1->convertTZ($AppUI->getPref('TIMEZONE'));
    ...
    foreach ($tasks as $row) {
        ...
        // change the color of the task accordingly
        if ($row['task_percent_complete'] == 100) { //HAS FINISHED
            $rowColor = "00FF00";
            $link['color'] = 'green';
        } elseif ($today2->after($end)) {
            $rowColor = "FF0000"; //LATE!!!
            $link['color'] = 'red';
        } elseif ($today2->before($end)) { //TASKS ENDING IN FUTURE
            if ($tommorow1->after($end)) { //WITHIN ONE DAY
                $rowColor = "FF9933";
                $link['color'] = 'orange';
            } else {
                $rowColor = "00FFFF";
        ...
        //if ($end && $end->after($startPeriod) && $end->before($endPeriod)) {
            if ($minical) {
                ...
                $temp = $link;
                if ($a != 'day_view') {
                    $temp['text'] = w2PtoolTip($row['task_name'], getTaskTooltip($row['task_id']), true) . ' ' . $temp['text'] . ' ' . w2PendTip(); //removed $end->format($tf) . which added the end time
                    $temp['text'].= '<a href="?m=tasks&amp;a=view&amp;task_id=' . $row['task_id'] . '&amp;tab=1&amp;date=' . $AppUI->formatTZAwareTime($row['task_end_date'], '%Y%m%d') . '">' . '</a>'; //Removed . w2PtoolTip('Add Log', 'create a new log record against this task') . w2PshowImage('edit_add.png') . w2PendTip()
                }
            }
            $links[$end->format(FMT_TIMESTAMP_DATE)][] = $temp;
        //}
...
function __extract_from_tasks5($q, $subquery)
{
    ...
    $q->addQuery('task_description, task_owner, task_status, task_updated');
...
function __extract_from_tasks_pinning($AppUI, $task_id)
{
    if (isset($_GET['pin'])) {
        ...
        if (-1 == $pin) {
            ...
            if (!$result) {
            $AppUI->setMsg('Pinning user, task: '.$task_id, UI_MSG_ERROR, true);
            }
		$task->load($task_id);
...
function __extract_from_view_messages4($s, $style, $row, $hideEmail, $editor, $AppUI, $new_messages, $bbparser, $m, $df, $tf, $canEdit, $canAdminEdit, $canDelete)
{
    ...
    $row['message_body'] = $bbparser->qparse($row['message_body']);
    $row['message_body'] = nl2br2($row['message_body']);
    $s .= $row['message_body'];
    $s .= '</td>';
...
function checkModuleItem_load($module, $item)
{
    switch ($module){
        case 'companies':
            $obj = new CCompany();
            return $obj->load($item);
            break;
        case 'contacts':
            $obj = new CContact();
            return $obj->load($item);
