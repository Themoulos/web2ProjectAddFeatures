...
abstract class w2p_Core_BaseObject extends w2p_System_Event implements w2p_Interfaces_Listener
{
    ...
    public function store($updateNulls = false)
    {
        ...
        $this->_recure = ($this->$k) ? 'Update' : 'Create';
        $this->_dispatcher->publish(new w2p_System_Event(get_class($this), 'pre' . $this->_recure . 'Event'));
    ...
    protected function hook_preDelete()
    {
        ...
        addHistory($this->_tbl, $this->{$this->_tbl_key}, 'Deleted', $name . ' - ' .
            $this->_AppUI->_('ACTION') . ': ' . 'Deleted' . ' ' . $this->_AppUI->_('TABLE') . ': ' .
    ...
    protected function hook_postStore()
    {
        ...
        $prefix = $this->_getColumnPrefixFromTableName($this->_tbl);
        $name = ('' != $this->{$prefix . '_name'}) ? $this->{$prefix . '_name'} : '';
        addHistory($this->_tbl, $this->{$this->_tbl_key}, 'Stored', $name . ' - ' .
            $this->_AppUI->_('ACTION') . ': ' .'Stored' . ' ' . $this->_AppUI->_('TABLE') . ': ' .
            $this->_tbl . ' ' . $this->_AppUI->_('ID') . ': ' . $this->{$this->_tbl_key});
