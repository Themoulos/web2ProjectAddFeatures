...
class w2p_Controllers_View {
    ...
    public function renderDeleteAllNextTasks() {
        $output = '';

        if ($this->object->canDelete()) {
            $output = "<script language=\"javascript\" type=\"text/javascript\">function delAllTasks()";
            $output .= "{if (confirm('" . $this->AppUI->_('doDelete') . ' \\nTask and ALL Incomplete Tasks in the Group?' ."')){";
            $output .= 'document.frmDeleteAllTasks.submit();';
            $output .= '}}</script>';
            $output .= '<form name="frmDeleteAllTasks" action="' . $this->action . '" method="post" accept-charset="utf-8">';
            $output .= '<input type="hidden" name="dosql" value="' . $this->dosql . '" />';
            $output .= '<input type="hidden" name="delAllTasks" value="1" />';
            $output .= $this->fields;
            $output .= '<input type="hidden" name="task_recure_id" value="' . $this->object->task_recure_id . '" />';
            $output .= '<input type="hidden" name="' . $this->key . '" value="' . $this->object->getId() . '" />';
            $output .= '</form>';
        }
