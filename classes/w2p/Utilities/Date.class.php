...
class w2p_Utilities_Date extends Date {
    ...
    public function addMonths($n) {
        ...
        if ($n < 0) {
            ...
            if ($this->month < 1) {
                ...
                $new_date = new w2p_Utilities_Date(Date_Calc::endOfMonth($this->month, $this->year, FMT_TIMESTAMP_DATE));
                $this->day = $new_date->day;
                $this->month = $new_date->month;
                $this->year = $new_date->year;
                }
                //if (!$this->isWorkingDay()) $this->prev_working_day(true);
        }
    }
    ...
    */
    public function next_working_day($preserveHours = false) {
        ...
        if ($preserveHours) {
            $this->setTime($do->getHour(), $do->getMinute(), '0');
        }
