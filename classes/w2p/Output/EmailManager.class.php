...
//TODO: customize these date formats based on the *receivers'* timezone setting
    ...
    public function getTaskNotify(CTask $task, $user, $workers, $projname, $comment)
    {
        $body = $this->_AppUI->_('Task Notification', UI_OUTPUT_RAW) . "\n";
        ...
        $body .= "\n\n". $this->_AppUI->_(' Assigned to Task', UI_OUTPUT_RAW) . ":\n";
        foreach ($workers as $row) {
            $body .=  "\t".$row['assignee_name'] . ', ' . $row['assignee_email'].",  \n";
        }
        
        if (isset($comment) && $comment != '') {
        $body .= "\n". $this->_AppUI->_('Additional Email Comments', UI_OUTPUT_RAW) . ":\n".$comment;
        }

        return $body;
    }

    public function getTaskNotifyOwner(CTask $task, $user, $workers, $projname, $comment)
    {
        $body = $this->_AppUI->_('Task Owner Notification', UI_OUTPUT_RAW) . "\n\n";
        $body .= $this->_AppUI->_('Project', UI_OUTPUT_RAW) . ":\t" . $projname . "\n";
        $body .= $this->_AppUI->_('Task', UI_OUTPUT_RAW) . ":\t\t" . $task->task_name . "\n";
        ...
        $body .= $this->_AppUI->_('Owner', UI_OUTPUT_RAW) . ":\t\t" . ' YOU ' . "\n";
        $body .= $this->_AppUI->_('Priority', UI_OUTPUT_RAW) . ":\t" . $task->task_priority . "\n";
        $body .= $this->_AppUI->_('Progress', UI_OUTPUT_RAW) . ":\t" . $task->task_percent_complete . "%\n";
        ...
        $body .= $this->_AppUI->_('Log-Comm.', UI_OUTPUT_RAW). ":\t".$comment;

        $user = new CUser();
        $user->load($this->_AppUI->user_id);
        $body .= "\n--\n" . $user->user_signature;
    ...
    {
        $body = $this->_AppUI->_('Task Reminder', UI_OUTPUT_RAW) . "\n\n";
    ...
    {
        $project = new CProject(); //THEMOULOS CHANGED THE NEXT FOUR LINES
        ...
        //$body = $this->_AppUI->_('Task Log Update', UI_OUTPUT_RAW) . "\n\n";
        $body = $this->_AppUI->_('Project', UI_OUTPUT_RAW) . ":\t" . $projname . "\n";
        $body .= $this->_AppUI->_('Task', UI_OUTPUT_RAW) . ":\t\t" . $task->task_name . "\n";
        if ($task->task_parent != $task->task_id) {
            $tmpTask = new CTask();
        ...
        $body .= $this->_AppUI->_('Log-Comm.', UI_OUTPUT_RAW) . ":\t" .$log->task_log_description;
